...
抽出物は `.local/` のみへ保存します。`catalog/game.json` は識別情報、原文ハッシュ、原文のSHA-256、変数・タグ・改行の制約だけを含みます。原文は収録しません。`catalog/supported-builds.json` はゲーム同梱の `Version.txt` と照合する対応版名です。インストーラーは大容量のゲームコンテナをハッシュせず、不一致または判定不能なら互換性を警告して続行確認を表示します。
...
導入テストはゲームと無関係な自作データを使います。WindowsではPowerShell 5.1の実処理を起動し、正常導入、再導入、更新、削除、未知・改変・欠落ファイル、対応版の一致、未対応版への警告と確認、書き込み失敗などを確認します。実機の通しプレイは別途必要です。
