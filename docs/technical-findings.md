...
自作した設定の実機読み込みとメニュー・設定画面の表示を確認しました。ローカルでIoStoreから読み戻したプロパティも生成時と一致しています。NotoSansJP適用後の字幕について、ユーザーから「読みやすくなった。表示も問題ない」と確認を得ました。今回確認した冒頭以外の表示は、通しプレイで引き続き検証します。
