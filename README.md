...
対応版は `1.0.0.225957_HellraiserGameDemo_Shipping_26254_Demo_Test` です。Windows版Steamデモで表示と通しプレイを確認しています。異なる版や判定できない環境では、互換性の警告と続行確認を表示します。
...
Python、.NET、解析ツールは不要です。Windowsに付属するPowerShellを使用します。ゲーム同梱の `Version.txt` で対応版を確認するため、ゲーム本体の大容量ファイルは検査しません。
...
- **未対応のゲーム版・版を判定できない場合:** 日本語が表示されない、表示が崩れる、またはゲームが起動しない可能性があります。警告内容を確認して続行するか、中止して対応版を確認してください。ゲーム本体は上書きされません。
